...
*   **Protocol Defense:** Minting 5,000 IDs is *cheap* — about 50 s on one CPU core, under a second on a GPU (Ch2 §2.2.2) — so PoW is not the defence. The defence is the **prefix cap**: $X$ admits no more than $5\%$ of its k-bucket and Active Set entries from one $/24$ (IPv4) or $/48$ (IPv6), so filling even one bucket needs $\ge 20$ distinct prefixes, and the disjoint-path lookup bound $P_{\text{hijack}} \le f^{\alpha}$ (Ch2 §2.1.2) holds with $f$ the *prefix* fraction the attacker controls.
...
*   **Protocol Defense:** Per-tree symmetry auditing detects bi-directional flow within one tree (impossible for honest parent→child edges) and IP subnet clustering discounts bundles; verified evidence of a symmetric pair can be gossiped as a signed accusation, and three ranked accusers from three prefixes evict the pair locally for a bounded period (Ch5 §5.3).
