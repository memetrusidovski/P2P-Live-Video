...
*   **No explicit difficulty field:** the adaptive $C_2$ tier (Ch2 §2.2.1) needs no wire encoding. A proof solved at a higher difficulty automatically satisfies every lower threshold, so the verifier simply checks the hash against the tier required for the swarm size *it* observes, accepting one tier below to tolerate stale swarm-size reads during rapid growth. That grace band is a **verifier tolerance, not a solver discount**: it is applied to $\text{tier}(N)$ before any class or reconnect reduction and never composed with them, and the effective requirement is floored at $C_2^{\min} = 8$ (Ch2 §2.2.1, "Discounts Do Not Stack"). The `NodeClass` claimed in the frame is what authorises the leaf rung, so a verifier must apply the discount only for the class actually declared.
