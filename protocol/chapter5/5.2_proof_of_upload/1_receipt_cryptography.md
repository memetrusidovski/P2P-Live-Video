...
$$\text{PoU} = \text{Sign}_{SK_B}\bigl(\text{SegmentSeq} \parallel \text{TreeID} \parallel \text{RxFlags} \parallel \text{LossRate} \parallel \text{BlockBitmap} \parallel K_s \parallel \text{NodeID}_A \parallel \text{NodeID}_B \parallel \text{Timestamp}\bigr)$$

*   **`BlockBitmap`** — bit $\text{ChunkIndex} \cdot 128 + j$ is set iff block $(\text{ChunkIndex}, j)$ of the segment (Ch4 §4.1.1) was received from $A$ and verified. The receipt's **byte value** is $16\text{ KB} \times \text{popcount}$; it is what the contribution score counts (§2).
