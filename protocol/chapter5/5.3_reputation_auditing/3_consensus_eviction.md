...
*   **Accusers must have standing, and the accusation carries it.** An accuser's rank is its $\Theta^{\text{rate}}/B$. A receiver knows it for a current tree child (which presented a `RANK_PROOF` at admission); for every other accuser — and gossip delivers accusations mostly from peers that are *not* the receiver's tree neighbours — the accusation frame **carries the accuser's own `RANK_PROOF`** (Appendix D §D.4.18). The proof is bound to the accuser because its `Prover NodeID` is the accuser's and the accusations are signed by the same key, and it is verifiable by anyone: its nonce is a source-signed manifest root, not anything receiver-specific, so a forwarder re-sends it unchanged. An accuser may omit the proof only when sending to a current parent or child. A receiver that cannot rank an accuser — no attached proof and no local view, or a proof whose nonce manifest it no longer holds — **does not count and does not forward** the accusation. A fresh identity with no contribution history cannot vote: a Sybil can be minted in 10 ms, but a Sybil with rank $0.25$ has relayed a quarter of a stream for a minute. An earlier draft defined the proof's only trigger as a tree join, so a gossip-received accusation was verified, forwarded, and counted by no one; the audit channel carried traffic no node could act on.
...
Attaching the proof bounds the channel's reach in time: the nonce manifest must be one the verifier holds or can fetch within $\tau_{\text{retain}} = 8$ s, so an accusation must be verified within a few seconds of its proof being made. HyParView gossip crosses the swarm in a few hops, well inside that; a receiver holding an accusation it could not verify in time drops it rather than forwarding a proof nobody downstream can check.

## Eviction Is Local and Bounded

Eviction is a **local** decision with a **bounded** duration: $\tau_{\text{ban}} = 10$ minutes on the first eviction, doubling on each repeat, capped at 24 hours, then forgotten. There is no swarm-wide permanent ban and no ban list is gossiped — only accusations are, and only with their evidence and their accuser's proof. A peer that receives an accusation and finds its evidence invalid **does not forward it**, and treats the accuser as having presented a forgery (local ban, Ch7 §7.1.1). A peer forwards only accusations it has itself counted. Lying about a peer therefore costs the liar its standing at every honest node that checks — the cost to accuse that the earlier design lacked.
