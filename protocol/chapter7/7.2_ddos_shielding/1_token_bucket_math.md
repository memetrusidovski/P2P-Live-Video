...
*   A per-source handshake budget ($\text{RATE\_PPS\_UNKNOWN} = 50$, burst $100$) in a separate LRU map. The map is keyed by IPv4 source address, and a carrier-grade NAT address is shared by thousands of viewers: at the earlier $10$ pps, a regional launch behind one CGNAT pool could register or query at $10$ peers per second per guardian, with everyone else dropped as a flood. $50$ pps covers a handful of simultaneous handshakes and pre-session DHT frames per address while the global ceiling below still bounds aggregate work.
...
**Guardian load fits under the ceiling only because registration is sampled.** A DHT guardian receives every `REGISTER_PEER` and `GET_PEERS` for its stream as pre-session UDP from unknown sources, all of it charged to `GLOBAL_NEW_PPS`. Un-sampled, a million-viewer stream would send each guardian $\approx 11{,}000$ registrations per second — more than twice this ceiling — and the guardian's own shield would drop over half of the legitimate registrations, silently, looking like churn. The registration sample of Ch2 §2.3.2 (target $10^4$ registrations regardless of $N$) and the query discipline there keep a guardian's unknown-source traffic in the low hundreds of packets per second at any scale.
